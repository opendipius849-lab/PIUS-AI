...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("QADEER-AI~", '');
  try {
...
async function connectToWA() {
  ...
  // CHANGE 1: Group Metadata Cache ko yahan initialize karein
...
│ 💻 *GitHub:* │   https://github.com/Qadeer-Xtech/QADEER-AI
╰─────────────╯
...
`;
  ...
  conn.ev.on("group-participants.update", (update) => {
      // CHANGE 1.1: Jab group mein koi change ho to cache clear karein
      if (groupMetadataCache.has(update.id)) {
          groupMetadataCache.delete(update.id);
          console.log(`Cache cleared for group: ${update.id}`);
      }
  ...
  conn.ev.on('messages.upsert', async (mek) => {
    ...
    // CHANGE 2: Message saving ko non-blocking banayein
    saveMessage(mek).catch(console.error); // `await` hata diya takey code iska intezar na kare
    ...
    // CHANGE 1.2: Metadata ko cache se hasil karein ya fetch kar ke cache mein save karein
    let groupMetadata;
    if (isGroup) {
        if (groupMetadataCache.has(from)) {
            groupMetadata = groupMetadataCache.get(from);
        } else {
            try {
                groupMetadata = await conn.groupMetadata(from);
                if (groupMetadata) groupMetadataCache.set(from, groupMetadata);
            } catch (e) {
                console.error("Failed to fetch group metadata:", e);
                groupMetadata = {}; // Agar error aye to empty object
            }
        }
    } else {
        groupMetadata = {};
    ...
    const jawad = ('923151105391', '923079749129', '923498344152');
    ...
    } else { // CHANGE 3: Agar command nahi hai to hi non-command events check karein
